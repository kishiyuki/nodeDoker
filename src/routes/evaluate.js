  ...
  if (!errors.isEmpty()) {
    ...
    connection.query('select * from users;', function(err, users) {
      connection.query('select * from events where id = ' + req.body.event_id + ';', function(err, events){
        ...
        var address = events[0].id.toString() + "_" + sender[0].toString() + "_" + receiver[0].toString();
        connection.query('select * from evaluates where event_id = ' + events[0].id + ' and sender_id = ' + sender[0].id + ' and receiver_id = ' + receiver[0].id + ';', function(err, evaluates) {
          if(evaluates != []){
